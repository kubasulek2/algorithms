...
class DayBuilder {
    ...
    build() {
        ...
        this.eventArr.forEach(ev => {
            ...
            const startIdx = ev.starts * 2, endIdx = ev.ends * 2, eventID = ev.id, event = { eventID, span: endIdx - startIdx, width: 0, leftOffset: 0 };
    ...
    calculateEventSize(event, row, column) {
        const { span } = event;
